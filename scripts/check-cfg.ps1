...
foreach ($file in $Path) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        elseif ($line -match '^echo\s') {
            ...
            foreach ($m in [regex]::Matches($line.ToUpper(), '\b(F1[0-2]|F[1-9]|HOME|END|INS|DEL|PGUP|PGDN|LEFTARROW|RIGHTARROW|UPARROW|DOWNARROW|KP_PLUS|KP_MINUS|KP_MULTIPLY|KP_DIVIDE)\b')) {
